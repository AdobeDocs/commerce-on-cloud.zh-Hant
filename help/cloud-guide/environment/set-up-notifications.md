...
根據預設，雲端基礎結構上的Adobe Commerce會將建置和部署動作寫入Adobe Commerce根應用程式目錄內的`app/var/log/cloud.log`檔案。 您可以選擇將記錄檔傳送至傳訊系統（例如Slack和電子郵件），以接收即時通知。

例如，您可以傳送Slack訊息，在部署失敗時提醒一組人員，並提示調查問題。
...
例如，在初始開發期間，您可能會偏好使用顯示整合環境詳細記錄的電子郵件通知，以協助您在部署到中繼環境之前對問題進行偵錯。 當您準備好部署至測試或生產環境時，您可能會偏好使用內含較不詳細資訊的Slack訊息。
...
- `channel` — 雲端基礎結構上的Slack頻道Adobe Commerce名稱會傳送通知。
- `username` — 雲端基礎結構上的Adobe Commerce使用者名稱用來在Slack中傳送通知訊息。
